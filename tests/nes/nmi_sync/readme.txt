...
setting bit 0 of $1 to enable monochrome mode. The time of the write
...
will fail sometimes. To verify timing, write an odd value to $1 after
...
The following demonstrates:
	...
	nmi:
		...
		; Instructions between nmi: and STA $4 must take an even
		; number of cycles. STA $4 must be done as a part of
		; synchronization.
		sta $4           ; count as 4 cycles
		...
...
Given the following NMI handler
	...
	nmi:
		...
		delay N cycles
		lda #$01
		sta $1   ; writes 2286+N+5 cycles into frame

The $1 write will be 2286+N+5 cycles after frame began. To have the
$1 write at a particular pixel, calculate N with
...
then the $1 write will occur at y=121 x=80.
...
The above applies to enabling monochrome mode by setting bit 0 of $1;
...
Given the following NMI handler
	...
	nmi:
		...
		delay N cycles
		lda #$01
		sta $1   ; writes 7471+N+5 cycles into frame

The $1 write will be 7471+N+5 cycles after frame began. To have the
$1 write at a particular pixel, calculate N with
...
13009, then the $1 write will occur no earlier than y=121 x=81.
...
* The NMI handler must not read $2 until after end_nmi_sync has been
called.
