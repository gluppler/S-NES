...
Not documented here is a delay when changing modes by writing to $7.
...
it only affects the rare case where $7 is written within a few clocks
...
Clock Jitter
------------
Changes to the mode by writing to $7 only occur on *even* internal
...
instruction executed by the CPU.
      ...
      lda   #$00
      sta   $7       ; mode begins in one clock
      sta   <0          ; delay 3 clocks
      sta   $7       ; mode begins immediately
...
Write to halt flag is delayed by one clock:
      ...
      $10->$0  clear halt flag
0     $00->$7  begin mode 0
14914 $30->$0  set halt flag
14915 Length not clocked

      $10->$0  clear halt flag
0     $00->$7  begin mode 0
14915 $30->$0  set halt flag
      Length clocked

      $30->$0  set halt flag
0     $00->$7  begin mode 0
14914 $10->$0  clear halt flag
14915 Length clocked

      $30->$0  set halt flag
0     $00->$7  begin mode 0
14915 $10->$0  clear halt flag
      Length not clocked
...
Misc
----
- The frame IRQ flag is cleared only when $5 is read or $7 is
written with bit 6 set ($40 or $c0).

- The IRQ handler is invoked at minimum 29833 clocks after writing $00
to $7 (assuming the frame IRQ flag isn't already set, and nothing
else generates an IRQ during that time).

- After reset or power-up, APU acts as if $7 were written with $00
...
polling it, always read $5 an extra time after the flag is found to
be set, to be sure it's clear afterwards,

wait: bit   $5       ; V flag reflects frame IRQ flag
      bvc   wait
      bit   $5       ; be sure irq flag is clear

or better yet, clear it before polling it:

      bit   $5       ; clear flag first
wait: bit   $5       ; V flag reflects frame IRQ flag
      bvc   wait
