...
| Forced Blanking | ✅ Yes | Rendering disabled via $1 |
...
|-----------|--------|-------|
| OAM DMA ($4) | 513 | Blocks CPU during transfer |
| Manual OAM Write | 2 per byte | Via $3/$4 |
...
| Operation | Cycles | Notes |
|-----------|--------|-------|
| Set PPU Address ($6) | 2 × 2 = 4 | High byte, then low byte |
| Write PPU Data ($7) | 2 per byte | Auto-increments address |
| Read PPU Data ($7) | 2 per byte | First read is buffered (invalid) |
...
| Operation | Cycles | Notes |
|-----------|--------|-------|
| Write $5 (X scroll) | 2 | First write |
| Write $5 (Y scroll) | 2 | Second write |
...
| Operation | Cycles | Notes |
|-----------|--------|-------|
| Strobe (write $6) | 2 × 2 = 4 | Write 1, then 0 |
| Read button | 2 per button | Read $6/$7 bit 0 |
