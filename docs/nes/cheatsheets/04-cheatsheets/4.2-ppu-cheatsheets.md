...
## PPU Register Reference ($0–$7)

| Address | Name | Write | Read | Bits |
|---------|------|-------|------|------|
| $0 | PPUCTRL | Control | — | N N I P P H B M M |
| $1 | PPUMASK | Mask | — | B G R s b M m G |
| $2 | PPUSTATUS | — | Status | V S O - - - - - |
| $3 | OAMADDR | OAM Address | — | A A A A A A A A |
| $4 | OAMDATA | OAM Data | OAM Data | D D D D D D D D |
| $5 | PPUSCROLL | Scroll | — | X X X X X X X X |
| $6 | PPUADDR | Address | — | A A A A A A A A |
| $7 | PPUDATA | Data | Data | D D D D D D D D |

### $0 (PPUCTRL) Bit Layout
...
**Note: Reading $2 clears VBlank flag and address latch.
...
|---------|------|-------------|
| $0–$23BF | 960 B | Tile map (32×30 tiles) |
...
|----------|-------|--------|
| $4 | High byte of OAM buffer | Starts DMA transfer |
| Duration | — | 513 CPU cycles |
| Source | $XX00–$XXFF | 256 bytes from CPU RAM |
| Destination | OAM ($4) | Sprite attribute memory |
...
- **$6 write sequence: High byte first, then low byte (resets after $2 read)
- **$7 first read: Returns buffered data (invalid), second read is valid
- **VRAM access: Only safe during VBlank or forced blanking
- **Scroll update: Must write $5 twice per frame (X, then Y)
- **Sprite limit: Maximum 8 sprites per scanline (hardware limit)
- **OAM corruption: Reading $2 during rendering can corrupt OAM
