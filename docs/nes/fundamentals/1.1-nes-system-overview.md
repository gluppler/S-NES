...
* **2 KB VRAM** (PPU-accessible video RAM, typically at $0–$3EFF)

The CPU and PPU operate on separate buses. The CPU cannot directly access PPU memory; all communication occurs through memory-mapped I/O registers ($0–$7).
...
| Register/State | Power-Up Value | Notes |
|----------------|----------------|-------|
| $0 (PPUCTRL) | Undefined | Random value |
| $1 (PPUMASK) | Undefined | Random value |
| $2 (PPUSTATUS) | Undefined | VBlank flag may be set |
| $3 (OAMADDR) | Undefined | Random value |
| $5 (PPUSCROLL) | Undefined | Random value |
| $6 (PPUADDR) | Undefined | Random value (address latch) |
| $7 (PPUDATA) | Undefined | Random value |
| VRAM ($0–$3EFF) | Undefined | Random values |
...
| Register/State | Power-Up Value | Notes |
|----------------|----------------|-------|
| $0–$3 (APU Channels) | Undefined | Random values |
| $5 (APU Status) | $00 | All channels disabled |
| $7 (Frame Counter) | Undefined | Frame counter not running |
| Channel Output | Silent | All channels disabled |

**Reset Behavior:**
- All APU registers are undefined
- Channels are disabled ($5 = $00)
...
* CPU memory map is fixed: $0000–$1FFF RAM (mirrored), $0–$3FFF PPU registers (mirrored), $0–$401F APU/I/O, $4020–$5FFF cartridge expansion, $6000–$7FFF cartridge RAM, $8000–$FFFF cartridge PRG ROM.
* PPU memory map: $0000–$1FFF pattern tables (CHR ROM/RAM), $0–$2EFF name tables (background), $3F00–$3F1F palettes, $3F20–$3FFF mirrors.
...
- **Frame Counter Reset**: Must write $7 at consistent time (in NMI)
...
reset:
    ...
    STX $0    ; Disable NMI (PPUCTRL = 0)
    STX $1    ; Disable rendering (PPUMASK = 0)
    STX $0    ; Disable DMC IRQ
...
9. **Scroll Latch: Must read $2 before writing scroll registers
...
**Check:**
1. Frame counter initialization ($7 written)
...
3. Sprite evaluation corruption ($2 reads)
