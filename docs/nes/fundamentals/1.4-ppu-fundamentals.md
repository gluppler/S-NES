...
The CPU communicates with the PPU exclusively through memory-mapped registers at $0–$7.
...
|----------------|----------------|-------------|-------|
| $0 (PPUCTRL) | Undefined | ❌ No | Random value |
| $1 (PPUMASK) | Undefined | ❌ No | Random value |
| $2 (PPUSTATUS) | Undefined | ❌ No | VBlank flag may be set |
| $3 (OAMADDR) | Undefined | ❌ No | Random value |
| $5 (PPUSCROLL) | Undefined | ❌ No | Random value |
| $6 (PPUADDR) | Undefined | ❌ No | Random value (address latch) |
| $7 (PPUDATA) | Undefined | ❌ No | Random value |
| VRAM ($0–$3EFF) | Undefined | ❌ No | Random values |
...
; PPU initialization sequence
    LDA #0
    STA $0    ; Disable NMI
    STA $1    ; Disable rendering
    
    ; Wait for first VBlank (PPU stabilization)
    BIT $2    ; Clear VBlank flag
vblank_wait1:
    BIT $2
    BPL vblank_wait1
...
* PPU register $0 bit 4 selects which pattern table is used for background
* PPU register $0 bit 3 selects which pattern table is used for sprites

### Name Tables

* Four name tables: $0, $2400, $2800, $2C00 (each 1 KB)
...
* PPU register $0 bits 0–1 select which name table is visible
...
* Sprites can be 8×8 or 8×16 pixels (PPU register $0 bit 5)
* Maximum 8 sprites per scanline (hardware limit)
* Sprite 0 hit flag ($2 bit 6) indicates sprite 0 overlaps background
...
* OAM is accessed via $3 (OAM address) and $4 (OAM data)
...
LDA $2    ; Reset address latch
LDA #$20
STA $6    ; High byte of PPU address
LDA #$00
STA $6    ; Low byte of PPU address

; Write to name table
LDX #0
loop:
    LDA name_table_data,X
    STA $7    ; Write to PPU, auto-increments address
    INX
...
load_palette:
    ...
    LDA $2           ; Read PPUSTATUS (clears VBlank flag, resets latch)
...
palette_loop:
    LDA palette_data,Y
    STA $7           ; Write to PPU data (auto-increments)
    INY
    CPY #32
    BNE palette_loop
...
load_background:
    ...
    ; Set PPU address to name table ($0)
    LDA #$20
    STA $6           ; Name table high byte
    LDA #$00
    STA $6           ; Name table low byte
...
clear_name_table:
    ...
    LDA $2           ; Reset latch
    LDA #$23
    STA $6           ; Attribute table high byte
    LDA #$C0
    STA $6           ; Attribute table low byte
...
2. **Latch Reset: Must read $2 before setting PPU address
3. **Address Sequence: Write high byte, then low byte to $6
4. **Auto-Increment: $7 auto-increments address after each read/write
5. **OAM DMA: OAM updates must use DMA ($4), not individual writes
...
- Maximum 8 sprites per scanline (hardware limit)
- Sprite overflow flag ($2 bit 5) is unreliable

**Sprite Evaluation Window:**
- Cycles 64-256 of each visible scanline
- **Reading $2 during this window corrupts OAM**
- Must avoid $2 reads during sprite evaluation
...
|----------|---------------|-------|
| $2 (PPUSTATUS) | Clears VBlank flag, resets address latch | Must read before $6 writes |
| $4 (OAMDATA) | Returns OAM byte at current address | OAM address increments |
| $7 (PPUDATA) | First read is buffered (invalid) | Subsequent reads return actual data |
...
**Critical Bug**: Reading $2 during sprite evaluation (cycles 64-256 of scanlines 0-239) **corrupts OAM**.
