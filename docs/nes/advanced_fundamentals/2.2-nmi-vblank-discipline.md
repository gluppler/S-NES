...
1. PPU register $0 bit 7 (NMI enable) is set
2. VBlank period begins (PPU status $2 bit 7 becomes 1)
...
During VBlank, the PPU is idle and VRAM can be safely accessed via $6/$7.
...
* **Read $2: Clears VBlank flag (prevents multiple NMI calls)
* **Update OAM: Use OAM DMA ($4) or manual writes ($3/$4)
* **Update VRAM: Write to name tables, palettes, pattern tables (if CHR RAM)
* **Update scroll registers: $5 (x, y scroll) and $6 (name table select)
...
* **Unnecessary register writes: Each $6/$7 write costs cycles
* **Missing $2 read: Must read $2 to clear VBlank flag
...
* **VRAM writes: ~2 cycles per byte (via $7)
* **Scroll updates: 5 cycles ($5 write × 2)
...
| Read $2 | 4 | Clear VBlank flag, reset latch |
| OAM DMA setup | 4 | Write $3, $4 |
| OAM DMA transfer | 513 | Hardware DMA (CPU blocked) |
| Scroll update | 5 | Write $5 twice |
| PPU control update | 2 | Write $0 |
...
- VRAM write cost: 2 cycles per byte (via $7)
...
* **Not reading $2: VBlank flag remains set, causing issues
...
* **Not resetting scroll latch: Must read $2 before scroll writes
...
### ❌ Anti-Pattern: Not Reading $2

```asm
; BAD: Doesn't read $2 (VBlank flag not cleared)
nmi:
    ; Missing: LDA $2
    LDA #0
    STA $3
...
```asm
; GOOD: Always read $2 first
nmi:
    LDA $2           ; MUST read first (clears flag, resets latch)
    ; ... rest of NMI ...
```
...
2. Missing $2 read (scroll latch not reset)
...
2. Scroll latch not reset (read $2 before writes)
...
nmi:
    ...
    LDA $2       ; MUST read to clear VBlank flag
    
    ; Update OAM (sprite data)
    LDA #0
    STA $3       ; OAM address = 0
    LDA #>oam_buffer
    STA $4       ; Start OAM DMA (513 cycles)
