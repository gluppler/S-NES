...
The PPU's internal address latch ($6) and data register ($7) have specific write sequences that must be followed exactly.
...
* **During forced blanking: VRAM writes are safe (rendering disabled via $1)

### $6/$7 Write Rules

**Setting PPU Address ($6):
1. First write: High byte of address (bits 14–8)
2. Second write: Low byte of address (bits 7–0)
3. Address latch is reset after reading $2
4. Address auto-increments by 1 or 32 after $7 read/write (controlled by $0 bit 2)

**Reading/Writing PPU Data ($7):
1. First read after $6 write returns **buffered data** (invalid)
2. Subsequent reads return actual PPU data
3. Writes go directly to PPU (no buffering)
4. Address increments automatically (by 1 or 32, depending on $0 bit 2)
...
* Scroll latch is reset after reading $2
* Must be written **twice per frame** (in NMI) to maintain scroll position

**Name Table Select ($6):
* Bits 0–1 of $0 select which name table is visible
* Writing to $6 also sets name table (bits 10–11 of address)
* Scroll and name table must be synchronized

### Sprite Evaluation Quirks

* **Sprite 0 Hit: Flag ($2 bit 6) is set when sprite 0 pixel overlaps background pixel
* **8 Sprite Limit: Maximum 8 sprites per scanline (hardware limit)
* **Sprite Overflow: Flag ($2 bit 5) is set when >8 sprites on a scanline (unreliable)
* **OAM Corruption: Reading $2 during rendering can corrupt OAM (sprite evaluation period)

### Safe VRAM Write Windows

* **VBlank: Scanlines 241–260 (NTSC), 241–310 (PAL) — **primary window
* **Forced Blanking: When $1 bit 3 = 0 (rendering disabled) — **always safe
...
|----------|--------------|---------------|-------|
| $0 (PPUCTRL) | 2 | Anytime | Controls NMI, pattern tables, name table |
| $1 (PPUMASK) | 2 | Anytime | Controls rendering enable, color emphasis |
| $2 (PPUSTATUS) | Read-only | Read anytime | Clears VBlank flag, resets address latch |
| $3 (OAMADDR) | 2 | VBlank only | Sets OAM write address |
| $4 (OAMDATA) | 2 | VBlank only | Writes to OAM (use DMA instead) |
| $5 (PPUSCROLL) | 2 | Anytime | Must write twice per frame (X, then Y) |
| $6 (PPUADDR) | 2 | VBlank only | Sets VRAM address (high, then low) |
| $7 (PPUDATA) | 2 | VBlank only | Reads/writes VRAM (auto-increments) |
...
When `$1` bit 3 (background enable) = 0 and bit 4 (sprite enable) = 0:
...
1. **First write to $5**: Stores **fine X scroll** (bits 0-2) and **coarse X scroll** (bits 3-7)
2. **Second write to $5**: Stores **fine Y scroll** (bits 0-2) and **coarse Y scroll** (bits 3-7)
3. **Reading $2**: Resets latch to X scroll (next write is X)

### Scroll Register Format

| Write | Bits 7-3 | Bits 2-0 | Meaning |
|-------|----------|----------|---------|
| 1st ($5) | Coarse X (0-31) | Fine X (0-7) | Horizontal scroll |
| 2nd ($5) | Coarse Y (0-29) | Fine Y (0-7) | Vertical scroll |
...
nmi:
    LDA $2        ; Reset scroll latch (MUST read first)
    
    ; Write scroll registers (must be done every frame)
    LDA scroll_x
    STA $5        ; First write = X scroll
    LDA scroll_y
    STA $5        ; Second write = Y scroll
...
**❌ Mistake: Not reading $2 before scroll update**
```asm
; BAD: Latch state unknown
    LDA scroll_x
    STA $5        ; May write to Y instead of X!
```
...
| OAM corruption window | 64-256 cycles into scanline | Reading $2 corrupts OAM |
...
wait_sprite0_hit:
    BIT $2        ; Check sprite 0 hit flag (bit 6)
...
**Critical**: Reading $2 during sprite evaluation (scanlines 0-239, cycles 64-256) **corrupts OAM**.

**Safe periods for $2 reads:**
...
**Best practice**: Only read $2 during VBlank or when rendering is disabled.

## PPU Address Latch Behavior

The PPU address latch ($6) has specific behavior:

1. **First write to $6**: Stores high byte (bits 14-8 of address)
2. **Second write to $6**: Stores low byte (bits 7-0 of address)
3. **Reading $2**: Resets latch (next $6 write is high byte)
4. **Writing $7**: Auto-increments address by 1 or 32 (controlled by $0 bit 2)

### Address Increment Mode

| $0 bit 2 | Increment | Use Case |
|-------------|-----------|----------|
...
; Set increment to +1 (default)
    ...
    LDA #$00
    STA $6
    ; Each STA $7 increments address by 1
```

### $7 Read Buffer Quirk

**Important**: The first read from $7 after setting $6 returns **buffered data** (invalid).

**Pattern:**
1. Write to $6 (set address)
2. First read from $7 → **Returns buffered data (ignore)**
...
; CORRECT: Read PPU data
    ...
    LDA #$00
    STA $6        ; Low byte
    LDA $7        ; First read = BUFFERED (ignore)
    LDA $7        ; Second read = ACTUAL data
    LDA $7        ; Third read = ACTUAL data
```
...
update_vram:
    LDA $2       ; Reset address latch (MUST read first)
...
name_loop:
    LDA name_data,X
    STA $7       ; Write to PPU (auto-increments)
    INX
    CPX #240
    BNE name_loop
    
    ; Update palette
    LDA $2       ; Reset latch again
    LDA #$3F
    STA $6       ; Palette address high
    LDA #$00
    STA $6       ; Palette address low
...
unsafe_update:
    LDA #$20
    STA $6       ; Writing during scanline 100 = CORRUPTION
    LDA #$00
    STA $6
    LDA #$42
    STA $7       ; This will corrupt the display
    RTS
```
