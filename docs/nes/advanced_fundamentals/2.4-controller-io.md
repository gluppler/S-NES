...
* **$6: Controller 1 (write: strobe, read: data)
* **$7: Controller 2 (read: data, also controls APU frame counter)

The CPU **strobes** the controller (writes 1 then 0 to $6 bit 0), which loads all 8 button states into an internal shift register. Then, the CPU reads $6/$7 eight times, receiving one button state per read (bit 0).
...
* **Not handling controller 2: $7 bit 0 for controller 2, bit 7 controls APU frame counter
...
* **Controller 2 not working: Must read from $7, not $6
