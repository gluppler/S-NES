...
1. **Pulse 1** ($0–$3): Square wave, variable duty cycle
2. **Pulse 2** ($4–$7): Square wave, variable duty cycle
3. **Triangle** ($8–$B): Triangle wave, no volume control
4. **Noise** ($C–$F): White noise generator
5. **DMC** ($0–$3): Delta Modulation Channel, 7-bit samples

The APU also has a **frame counter** ($7) that generates timing signals for:
...
|---------|-----------|----------|----------------|-------|
| Pulse 1 | $0–$3 | Square (12.5%, 25%, 50%, 75%) | Yes (envelope) | Can sweep frequency |
| Pulse 2 | $4–$7 | Square (12.5%, 25%, 50%, 75%) | Yes (envelope) | Can sweep frequency |
| Triangle | $8–$B | Triangle | No (linear counter only) | No volume control |
| Noise | $C–$F | White noise | Yes (envelope) | 15-bit or 7-bit LFSR |
| DMC | $0–$3 | 7-bit samples | Yes (sample data) | Requires DMA |
...
* **Frame counter synchronization: Must write to $7 at specific times
...
nmi:
    ...
    STA $7       ; Resets frame counter to frame 1
...
* **Write timing: Must write to $7 at start of frame (in NMI)
* **Synchronization: Writing $7 resets frame counter

### Frame Counter Write Timing

**Critical timing**: Write to $7 must occur at a consistent point each frame.

**Best practice**: Write in NMI handler, immediately after reading $2.

```asm
nmi:
    LDA $2       ; Clear VBlank flag
    
    ; Reset frame counter (timing-critical)
    LDA #%11000000  ; 5-step mode, IRQ inhibit
    STA $7       ; Must write at consistent time
...
* **Sample rate: 4.18 kHz to 33.5 kHz (controlled by $0)
...
| $0 bits 3-0 | Sample Rate (Hz) | Period (CPU cycles) | CPU Impact |
|----------------|------------------|---------------------|------------|
| $0 | 4,180 | ~428 | 1.7% |
| $1 | 4,726 | ~379 | 1.9% |
| $2 | 5,263 | ~340 | 2.1% |
...
- Can be disabled (infinite length) via bit 5 of $3/$7
...
**Linear Counter:**
- Controlled by $8 bit 7 (halt flag) and bits 0-6 (reload value)
...
- Sample rate controlled by $0
- Loop flag ($0 bit 6): Repeats sample when done

**Best practice**: Keep samples short and sample rate low to minimize CPU impact.

## Common Mistakes and Incorrect Assumptions

* **Not initializing frame counter: $7 must be written to start frame counter
...
* **Writing $7 at wrong time: Frame counter timing is critical
...
init_audio:
    LDA #$00
    STA $5       ; Disable all channels first
    
    LDA #%11000000  ; 5-step mode, IRQ inhibit
    STA $7       ; Initialize frame counter
...
**Check:**
1. Frame counter initialized ($7 written)
2. Channels enabled ($5 bits set)
3. Triangle linear counter set ($8)
...
**Check:**
1. Frame counter reset every frame (write $7 in NMI)
...
**Check:**
1. Linear counter reload value ($8 bits 0-6)
2. Linear counter halt flag ($8 bit 7)
3. Length counter (if enabled)
4. Channel enabled ($5 bit 2)
...
play_note:
    ...
    STA $3       ; Write again to set length counter
...
update_audio:
    ...
    ; Just ensure $7 was written in NMI
