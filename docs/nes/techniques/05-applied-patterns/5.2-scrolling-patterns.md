...
**Pattern: Update scroll X register ($5) each frame based on camera position. Load new tiles when scrolling reveals new columns.
...
**Pattern: Update scroll Y register ($5) each frame based on camera position. Load new tiles when scrolling reveals new rows.
...
* **Name table switching: Must update $0 bits 0–1 when crossing boundaries
