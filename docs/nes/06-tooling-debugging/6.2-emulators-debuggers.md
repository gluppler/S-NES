...
1. Set breakpoint at OAM DMA ($4 write)
2. Inspect OAM buffer ($0200–$02FF)
3. Check sprite Y position (should be 0–239)
4. Check PPU mask ($1) — sprites enabled?

**Find why tile is wrong:
1. Set breakpoint at VRAM write ($7 write)
2. Inspect PPU address ($6)
...
**Graphics corruption:
1. Check VRAM writes during rendering (breakpoint at $7 during scanline 0–239)
2. Check OAM corruption (inspect OAM buffer)
3. Check palette corruption (inspect $3F00–$3F1F)

**Input not working:
1. Breakpoint at controller read ($6)
2. Check if strobe is set (write $01 then $00 to $6)
