...
SNES animation systems for managing sprite and tile animations with frame tables, timers, and state-driven approaches.
...
; Entity animation state (WRAM)
entity_anim_frame = $7E2000  ; Current frame index (per entity)
entity_anim_timer = $7E2100  ; Frame timer (per entity)
entity_anim_id = $7E2200     ; Current animation ID (per entity)
...
**Implementation**:
```asm
; Timer-based animation (WRAM)
entity_anim_timer = $7E2100  ; Frames until next animation frame
...
; Entity state (WRAM)
entity_state = $7E2300      ; Current state (per entity)
entity_state_prev = $7E2400  ; Previous state (per entity)
...
- [Sprite Engine Patterns](5.1-sprite-engine-patterns.md) - Sprite management
